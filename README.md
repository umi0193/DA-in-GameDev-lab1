...
### Реализовать на сцене генерацию n кубиков. Число n вводится пользователем после старта сцены.
 ...
 Так как я не знакома с синтаксисом C# и Unity, не получилось реализовать в полном обьеме. Поэтому я пишу ход работы таким, как я его вижу, исходя из алгоритма действий. 
...
Поискав о том, как это реализовать, я смогла найти примерный скрипт, но реализовать и адаптировать его под заачу не получилось:
 ...cs
...
Предполагаю, что тут необходимо определить, что enemy - это куб, а в цикле for - задать переменную, которая будет передавать значения из инпута.                           
